...
//
//              // check s values greater than mine and the s gap < 30
...
//          }
//            // use the previous paths end point as start reference
//          else
//          {
//
...
//          }
//
//          // In Frenet add evenly 30m spaced points ahead of the start reference
//
...
//          {
//            // shift carr ref angle to 0 degrees
...
//          // create a spline
//          tk::spline s;
//          //std::cout<<anchor_pts_x.size()<<anchor_pts_y.size()<<std::endl;
//
...
//
//          //Start with all the previous paths from last time
...
//          }
//
//          // Calculate how to break up spline points so that we travel at our destined reference velocity
...
////            for(int i = 0; i < 100; i++)
////            {
////              double next_s = car_s+(dist_inc*i)*cos(deg2rad(car_yaw));
////              double next_d = 6;
...
int main() {
  ...
  h.onMessage([&bp, &pc, &intended_velocity, &map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
                                                                                                                           uWS::OpCode opCode) {
    // "42" at the start of the message means there's a websocket message event.
    // The 4 signifies a websocket message
    // The 2 signifies a websocket event
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          //Number of way points left for previous set of path planner points
          ...
          */
          pc.Init(sensor_fusion);
          pc.DetectObjects(car_s, lane_id, previous_size);
          ...
          */
          bp.DefiningBehavior(pc, intended_velocity, lane_id);
